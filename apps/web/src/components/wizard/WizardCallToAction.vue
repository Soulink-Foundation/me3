...
const canEnableGifts = computed(() => stripeConnectStatus.value === "active");

const giftEnabled = computed({
  get: () => wizard.profile.gift.enabled,
  set: (val: boolean) => {
    if (val && !canEnableGifts.value) return;
    wizard.setGift({ enabled: val });
    if (val && !wizard.profile.gift.title.trim()) {
      wizard.setGift({ title: "Send a gift" });
    }
  },
});

const giftTitle = computed({
  get: () => wizard.profile.gift.title,
  set: (val: string) => wizard.setGift({ title: val }),
});

const giftDescription = computed({
  get: () => wizard.profile.gift.description,
  set: (val: string) => wizard.setGift({ description: val }),
});

const giftSuggestedAmount = computed({
  get: () => wizard.profile.gift.suggestedAmount,
  set: (val: number) => {
    const amount = Number.isFinite(val) ? Math.max(5, Math.round(val)) : 5;
    wizard.setGift({ suggestedAmount: amount });
  },
});

const giftCurrency = computed({
  get: () => wizard.profile.gift.currency,
  set: (val: "USD" | "GBP" | "EUR" | "CAD" | "AUD" | "CHF" | "SGD" | "INR" | "PKR") => wizard.setGift({ currency: val }),
});

const giftIcon = computed({
  get: () => wizard.profile.gift.icon || "",
  set: (val: string) => wizard.setGift({ icon: val }),
});

const giftStatusHint = computed(() => {
  if (!stripeConnectLoaded.value) {
    return "Checking Stripe connection...";
  }
  if (stripeConnectStatus.value === "restricted") {
    return "Finish Stripe setup to enable gifts.";
  }
  if (stripeConnectStatus.value === "pending") {
    return "Stripe setup is pending. Finish onboarding to enable gifts.";
  }
  if (stripeConnectStatus.value !== "active") {
    return "Connect Stripe to enable gifts.";
  }
  return "";
});
...
async function checkStripeConnectStatus() {
  try {
    ...
    stripeConnectStatus.value =
      (data.status as typeof stripeConnectStatus.value) || "not_connected";
  } catch (error) {
...
async function connectStripe() {
  if (isConnectingStripe.value) return;
...
async function disconnectStripe() {
  if (isDisconnectingStripe.value) return;
  if (
    !window.confirm(
...
watch(stripeConnectStatus, (status) => {
  if (status !== "active" && wizard.profile.gift.enabled) {
    wizard.setGift({ enabled: false });
  }
});
    ...
    <div class="gift-card">
      <div class="gift-card-header">
        <div class="gift-card-main">
          <UiIcon name="Gift" :size="28" aria-hidden="true" />
          <div class="gift-text">
            <span class="gift-title">Gifts</span>
            <span class="gift-desc">
              Accept one-time gifts via Stripe Connect.
            </span>
          </div>
        </div>
        <div class="gift-actions">
          ...
          <label class="gift-toggle" :class="{ disabled: !canEnableGifts }">
            <input
              type="checkbox"
              v-model="giftEnabled"
              :disabled="!canEnableGifts"
            />
            <span class="gift-toggle-ui" />
          </label>
        </div>
      </div>
      <div
        v-if="stripeConnectStatus !== 'active'"
        class="gift-country-field"
      >
        <label for="gift-stripe-country">Payout country</label>
        <select
          id="gift-stripe-country"
      ...
      <p v-if="giftStatusHint" class="gift-hint">{{ giftStatusHint }}</p>

      <div v-if="giftEnabled" class="gift-settings">
        <div class="form-group">
          <label>Icon <span class="optional">(optional)</span></label>
          <div class="icon-picker-row">
            <IconPicker v-model="giftIcon" />
            <span class="icon-picker-hint">Choose an icon or emoji</span>
          </div>
        </div>
        <div class="form-group">
          <label>Gift button text</label>
          <input
            v-model="giftTitle"
            type="text"
        ...
        <div class="form-group">
          ...
          <input
            ...
            type="text"
            placeholder="Offer a gift to support my work"
            maxlength="120"
          />
        </div>
        <div class="gift-row">
          <div class="form-group">
            ...
            <input
              v-model.number="giftSuggestedAmount"
              type="number"
              min="5"
              step="1"
            />
